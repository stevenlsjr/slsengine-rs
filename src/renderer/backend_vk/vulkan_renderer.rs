    ...
    pub fn draw_frame(&self, window: &Window, world: &EntityWorld<Self>) {
...

        {
            ...
            for &(entity, mask) in system.entities.iter() {
                let model: Mat4 = system.transforms[entity.0]
                    .clone()
                    .unwrap()
                    .transform
                    .into();
                ...
                let mesh_handle =
                    system.meshes.get(*entity).map(|c| c.mesh).unwrap();
                let VkMesh {
                    ref index_buffer,
                    ref vertex_buffer,
                    ..
                } = world.resources.fetch(mesh_handle).unwrap();
...
impl<'a> RenderSystem<'a> {
    fn collect(world: &'a EntityWorld<VulkanRenderer>) -> Self {
        let required_mask: ComponentMask =
            ComponentMask::MESH | ComponentMask::TRANSFORM;
        ...
        RenderSystem {
            entities: world
                ...
                .entities()
                .flat_map(|e| {
                    let mask = world.components.masks[*e]
                        .unwrap_or(ComponentMask::NONE);
