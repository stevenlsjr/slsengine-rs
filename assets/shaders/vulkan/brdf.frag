...
const float ambient_factor = 0.04;
...
float
smith_geometry_schlick_ggx(float n_dot_v, float roughness)
{
...
smith_geometry(vec3 normal, vec3 V, vec3 L, float roughness)
{
  ...
  float ggx2 = smith_geometry_schlick_ggx(n_dot_v, roughness);
  float ggx1 = smith_geometry_schlick_ggx(n_dot_l, roughness);
  return ggx2 * ggx1;
}
